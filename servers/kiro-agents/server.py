...
CHILDREN = {
    ...
    "c5-internal": (
        "Investigate using your FULL toolkit: customer account queries (k2), SOPs, Dante scripts, "
        "CCS context, internal wikis, Atlas, tickets, and AWS documentation.\n\n"
        "## MANDATORY STEP 1 — Customer Account Data (k2)\n"
        "If the topic mentions a customer account ID, query their CloudWatch metrics FIRST:\n"
        "- Use k2_search_for_aws_api to find the CloudWatch getMetricStatistics and getMetricData APIs\n"
        "- Pull ALL relevant metrics with extended statistics (p50, p90, p99) at hourly granularity\n"
        "- Cover the FULL complaint window PLUS 3 days before AND after\n"
        "- For Bedrock cases: TTFT, InvocationLatency, InputTokenCount, OutputTokenCount, "
        "CacheRead/WriteInputTokenCount, InvocationThrottles, EstimatedTPMQuotaUsage, Invocations\n"
        "- Check for behavior changes: compare before vs after complaint date\n\n"
        "## MANDATORY STEP 2 — Runbooks & Fleet Context\n"
        "- Use sop_search and sop_get to find relevant runbooks for the service\n"
        "- Use dante_search_tools to find diagnostic scripts\n"
        "- Search for fleet-wide dashboards, E2M alarm history, Harbinger events\n"
        "- Check events in ADJACENT days, not just the complaint window\n\n"
        "## STEP 3 — Internal Sources\n"
        "- Use InternalSearch for wikis, Sage, broadcasts, known issues\n"
        "- Use TicketingReadActions for related incidents and past cases\n"
        "- Use Atlas for service architecture context\n"
        "- Use ccs_get_harbinger_events for active service events\n\n"
        "Topic: {description}\n\nWrite findings to: {findings_path}"
    ),
}
...
VALIDATOR_TASK = (
    ...
    "When validating latency claims, check if the investigation queried the customer's actual "
    "CloudWatch metrics or relied only on a customer-provided CSV. If only CSV was used, mark "
    "the finding as UNVERIFIED with note: 'Customer CloudWatch not queried — CSV is a biased sample.'\n\n"
...
ORCHESTRATOR_TASK = (
    ...
    "## DATA COMPLETENESS CHECK (Do This First)\n"
    "Before writing the report, verify that c5-internal collected:\n"
    "1. Customer CloudWatch metrics (TTFT, InputTokenCount, OutputTokenCount, Cache metrics, Throttles)\n"
    "2. Fleet-wide context (E2M alarms, Harbinger events, production dashboards)\n"
    "3. Metrics for the FULL window including days AFTER the complaint (customer may have changed behavior)\n"
    "If ANY of these are missing, use your own tools to fill the gaps before proceeding.\n"
    "DO NOT write a report based only on a customer-provided CSV or case notes without CloudWatch data.\n\n"
